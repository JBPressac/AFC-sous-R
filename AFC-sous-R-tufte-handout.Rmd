...
La définition du répertoire de travail peut aussi être faite par le biais de la commande `Set As Working Directory` via l'onglet `More`^[Emplacement de l'onglet `More`![](Roue.png "Roue crantée dans la fenêtre en bas à droite de RStudio")]. Cet onglet correspond au symbole de la roue crantée dans la fenêtre en bas à droite de RStudio.
...
Nous pouvons, très facilement et à n'importe quel moment, afficher le tableau de contingence `wfemmes` à partir du volet Data^[Emplacement du tableau de contingence![](Data1.png "Volet Data dans la fenêtre en haut à droite de RStudio")] dans la fenêtre en haut à droite de RStudio.
...
Nous pouvons voir que, dans le volet Data dans la fenêtre en haut à droite de RStudio, le nombre de variables affichées a été réduit^[Réduction du nombre de variables![](Data2.png "Volet Data dans la fenêtre en haut à droite de RStudio")].
...
wf_marges <- wfemmes
wf_marges$Total <- rowSums(wf_marges)
wf_marges[nrow(wf_marges)+1, ] <- colSums(wf_marges)
row.names(wf_marges)[nrow(wf_marges)] <- "Total"
wf_marges
```
...
```
```{r, echo=FALSE}
kable(wfemmes_tableau_de_probabilite)
```
...
```{r khi2wfemmes$observed, include=FALSE}
khi2wfemmes$observed
```
```{r, echo=FALSE}
kable(khi2wfemmes$observed)
...
Pour plus de détails, nous vous recommandons la lecture de ce document^[Barnier J. (2016), _Tout ce que vous n'avez jamais voulu savoir sur le Khi2 sans jamais avoir eu envie de le demander_, https://alea.fr.eu.org/pages/khi2].

```{r khi2wfemmes$expected, include=FALSE}
khi2wfemmes$expected
```
```{r, echo=FALSE}
kable(khi2wfemmes$expected)
...
Il est important de rappeler que les résultats de cette enquête ont été publiés en 1974^[Rappel du tableau de contigence![](Tableau_Tabard.png "Tableau de contingence issu de l'enquête de N. Tabard de 1974")]. Il est fort à parier que la répartition des réponses serait totalement, si ce n'est en grande partie, différente aujourd'hui.
...
```{r plot(afcwfemmes)}
plot(afcwfemmes, cex = 0.7, cex.axis = 0.6, cex.lab = 0.8,
...
barplot(afcwfemmes$eig[,1], main="Histogramme des valeurs propres", 
       ...
       xlab = "Axes", ylab = "Pourcentage d'inertie",
       cex.axis = 0.8, cex.names = 0.8, col ="orange",
       names.arg = 1:nrow(afcwfemmes$eig))
```
...
barplot(afcpcsregion$eig[,1], main="Histogramme des valeurs propres",
       ...
       xlab = "Axes", ylab = "Pourcentage d'inertie",
       cex.axis = 0.8, cex.names = 0.8, col ="orange",
       names.arg = 1:nrow(afcpcsregion$eig))
```
