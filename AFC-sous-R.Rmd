...
![](Tableau_Tabard1.png "Tableau de contingence issu de l'enquête de N. Tabard de 1974")

Comme le souligne François Husson dans le MOOC, il est difficile de savoir à partir de ce tableau si les femmes sont favorables ou non au travail féminin. En effet, une majorité de femmes (908 sur 1 724, soit 52 %) ont répondu que la famille idéale est celle où "seul le mari travaille". Elles sont néanmoins 1 123 sur 1 724 (65 %) à avoir répondu que l'activité convenant le mieux à une mère de famille quand ses enfants vont à l’école est de travailler à mi-temps. L'AFC va nous permettre d'étudier le lien entre ces deux questions et de lever cette apparente contradiction. Elle va notamment nous permettre de visualiser la nature de la liaison entre les deux questions. Mais qu'est ce qu'une liaison ?       
...
(dimnames(don)[[2]][3]<-"Travail à plein-temps")
```
...
Comme le résultat est significatif, tout en respectant les conditons de validité du test du khi2, nous stockons le résultat de la fonction dans la variable `khi2don`.
...
Sachant qu'une liaison correspond à **l'écart entre les données observées et le modèle d'indépendance**, nous souhaitons donc **visualiser la nature de l'écart à l'indépendance entre deux variables qualitatives**.
...
Enfin, l'AFC travaille, non pas sur le tableau de contingence, mais sur le tableau de probabilité.     
...
Lors du précédent test du khi2, nous avons obtenu une p-value inférieure à 2.2e-16. Nous avons donc rejeté l’hypothèse d’indépendance entre les deux variables et admis que la liaison entre ces deux variables est signficative.       
...
Afin de faciliter, par la suite, la lecture et l'emploi des résultats cette AFC, nous les **stockons dans la variable** `res`.
...
![](Tableau_Tabard2.png "Tableau de contingence issu de l'enquête de N. Tabard de 1974")
...
Du coup, à ce stade de l'interprétation, nous nous retrouvons _a priori_ face une **contradcition**.       
...
```{r plot(res)}
plot(res, cex = 0.6, cex.axis = 0.6, cex.lab = 0.8, title = "Représentation graphique", selectRow = "cos2 0.7", selectCol = "cos2 0.7")
```     
...
Ce en sont que quelques arguments, parmi d'autres, d'améliorer la représentation graphique. Dans le jeu de données qui nous concerne, ces améliorations restent mineures et n'ont que peu d'impact sur la représentation graphique. Par contre, dans le cas où nous aurions à travailler sur un tableau de contingence, dont chacune des variables comporte au moins une vingtaine de modalités, ces améliorations graphiques prennent d'autant plus d'importance.       

Prenons un exemple pour illustrer notre propos. Nous allons nous appuyer sur des données de l'INSEE concernant les [spécificités socioprofessionnelles régionales](https://www.insee.fr/fr/statistiques/1281090#consulter "spécificités socioprofessionnelles régionales").       

Importons le jeu de données.       

```{r read.table(pcsregion)}
pcsregion <- read.table("pcsregion.csv", header=TRUE, row.names=1, sep=";", check.names=FALSE, fileEncoding="latin1")
```
...
À la lecture de ce tableau de contingence, nous disposons de 8 modalités en colonne et 23 modalités en ligne. 

Lorsque nous réalisons une AFC avec ce jeu de données, nus obtenons ceci :       

```{r CA(pcsregion)}
afcpcsregion <- CA(pcsregion)
```

Avec "seulement" 30 modalités,  nous voyons que le graphe est (en grossissant légèrement le trait) "totalement" illisible.      

```{r plot(res)}
plot(afcpcsregion, cex = 0.6, cex.axis = 0.6, cex.lab = 0.8, title = "Représentation graphique", selectRow = "cos2 0.7", selectCol = "cos2 0.7")
```  
...
Les tableaux avec les résultats permet d'obtenir :
...
* Dans l'exemple qui nous concerne ce n'est utile mais, dans un certain nombre de cas, cette argument est utile.       
...
Comme nous avons pu le voir, la fonction `summary()` nous permet d'obtenir des tableaux avec les résultats suivants :       

* Le test du khi2 (uniquement sur les lignes et les colonnes actives) avec la p-value.
* Les valeurs propres, les pourcentages d'inertie associés à chaque dimension.
* Les résultats sur les lignes actives avec leur coordonnée (dim.n) sur chaque dimension, leur contribution à la construction (ctr) de chaque dimension et leur qualité de réprésentation (cos2) sur chaque dimension.
* Les résultats sur les colonnes actives (dim.n, ctr, cos2).
* Les résultats (optionnel) sur les éléments supplémentaires en ligne avec la coordonnée (dim.n) et la qualité de représentation (cos2).
* Les résultats (optionnel) sur les éléments supplémentaires en colonne avec la coordonnée (dim.n) et la qualité de représentation (cos2).




##### Analyse d'une AFC en 5 étapes

* 1 - Détermination du nombre d'axes à interpréter
* 2 - Interprétation des coordonnées des fréquences actives et des individus actifs
* 3 - Interprétation de la contribution et de la qualtité de représentation des fréquences et des ind.
* 4 - Élaboration de la typologie
* 5 - Éléments d'interprétation

###### ÉTAPE 1 - DÉTERMINATION DU NOMBRE D'AXES À INTERPRÉTER       

Les valeurs propres (i.e. variance) des axes       

```{r Valeur propre des axes(res)}
res$eig
```

Dans notre exemple, on obtient 2 facteurs, classés du plus importants au moins importants (du plus explicatif au moins explicatif). Chacun des facteurs a une "valeur propre" (quantité d'information
expliquée par le facteur). La deuxième colonne renvoie au "pourcentage d'inertie", correspondant à la proportion des variations totales des variables originales expliquées par chaque facteur.

Représentation de l'histogramme des valeurs propres     

```{r barplot(res)}
barplot(res$eig[,1], main = "Histogramme des valeurs propres", xlab = "Axes", ylab = "Pourcentage d'inertie", cex.axis = 0.8, cex.names = 0.8, col ="orange", names.arg = 1:nrow(res$eig))
```
...
L'histogramme va servir visuellement à déterminer le nombre d'axes à interpréter à l'aide du critère du "coude" de Cattell. Lorsqu'on observe un décrochement entre 2 facteurs, on détermine les facteurs avant ce décrochement comme étant les axes à interpréter. Ce n'est pas la seule règle possible. Un un autre critère de choix (appelé « critère de Kaiser ») suggère de garder tous les facteurs dont la valeur propre est supérieure à la valeur propre moyenne (qui vaut par définition 1 divisé par le nombre de facteurs). Pour plus d'informations, nous vous invitons à vous reporter au lien suivant afin d'approfondir ce travail quant au [nombre de facteurs à retenir](http://quanti.hypotheses.org/930/ "Histogramme des valeurs propres").       

###### ÉTAPE 2 - INTERPRÉTATION DES COORDONNÉES DES FRÉQUENCES ACTIVES ET DES INDIVIDUS ACTIFS       

Retour sur le tableau des résultats de l'AFC (chercher comment exporter ces résultats en .csv ou .txt).        

* "Il est seulement nécessaire de retenir les individus et fréquences (ou variables) dont les coordonnées s'écartent de l'origine des axes. L'AFC se construit sur la base d'une opposition entre individus, les individus proches de l'origine des axes sont assimilés à un profil moyen que l'analyse factorielle ne permet pas de dissocier. Une formule mathématique peut être proposée pour identifier les individus ou variables les plus remarquables : Coordonnées = racine carrée de la valeur propre." (Busca & Toutain, 2009)       

Coordonnées des modalités en ligne (...)       

* La valeur propre de l'axe 1 est de .... La racine carré de la valeur propre de l'axe 1 est de ...
* La valeur propre de l'axe 2 est de .... La racine carré de la valeur propre de l'axe 1 est de ...

Nous retiendrons donc les modalités avec des coordonnées supérieures à la racine carré de la valeur propre de chacun des axes retenus       

* Axe 1 : ...
* Axe 2 : ...

Coordonnées des modalités en colonne (...)       

* Axe 1 : ...
* Axe 2 : ...       

###### ÉTAPE 3 - INTERPRÉTATION DE LA CONTRIBUTION ET DE LA QUALITÉ DE REPRÉSENTATION       

Rappel de la représentation graphique       

```{r plot2(res)}
plot(res, cex = 0.6, title = "Représentation graphique")
```

Représentation graphique des lignes et des colonnes avec un cos2 supérieur à 0.7       

```{r plot3(res)}
plot(res, cex = 0.6, title = "Représentation graphique", selectRow = "cos2 0.7", selectCol = "cos2 0.7")
```

Mais problème. Pourquoi la limite du cos2 à 0.7 ?

Qualité de représentation d'une modalité (un "profil") selon un axe : coS2
C'est une indication de la représentation par l'axe de l'écart de la modalité par rapport au profil moyen.
...
CONTRIBUTION À LA CONSTRUCTION DES AXES
...
res$row$contrib
Affichage de la première colonne : les contributions au deuxième axe (affiché "Dim2")
res$row$contrib[,2]
Trier les contributions par ordre décroissant : 
sort(res$row$contrib[,2], decreasing = TRUE)
Affichage de la première ligne : les contributions de la modalité "Les deux conjoints 
travaillers également" aux axes 1 et 2 (affichés "Dim1" et "Dim2")
res$row$contrib[1,]


AFC - DÉCRIRE LES DIMENSIONS FACTORIELLES
dimdesc(res)
