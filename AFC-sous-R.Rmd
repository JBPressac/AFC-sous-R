...
En vue de compléter notre explication, nous réalisons un jeu de données à part avec les sommes marginales (marge ligne et marge colonne).       

```{r dons}
dons <- don[1:3]
dons$Total <- rowSums(dons)
dons[nrow(dons)+1, ] <- colSums(dons)
row.names(dons)[nrow(dons)] <- "Total"
View(dons)
```
