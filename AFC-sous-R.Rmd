...
Si vous avez des commentaires à formuler ou des remarques à effectuer, vous pouvez nos contacter via nos adresses mails (Jean-Baptiste.Pressac@univ-brest.fr, laurent.mell@univ-brest.fr). Par la même occasion, si vous souhaitez échanger plus en détail et/ou contribuer à cette démarche, nous sommes aussi disponible.
...
```{r dons_avec_marges}
don_avec_marges <- don
don_avec_marges$Total <- rowSums(don_avec_marges)
don_avec_marges[nrow(don_avec_marges)+1, ] <- colSums(don_avec_marges)
row.names(don_avec_marges)[nrow(don_avec_marges)] <- "Total"
don_avec_marges
```


Affichons ce tableau de manière plus agréable dans ce document (avec la fonction `kable()` du package `knitr`) :

```{r}

kable(don_avec_marges) 
```

Il est aussi intéressant de calculer les pourcentages en ligne et les pourcentages en colonne.

```{r dons_pourcentage_en_ligne}
dons_pourcentage_en_ligne <- don
dons_pourcentage_en_ligne$Moyenne <- rowMeans(dons_pourcentage_en_ligne)
dons_pourcentage_en_ligne[nrow(dons_pourcentage_en_ligne)+1, ] <- colSums(dons_pourcentage_en_ligne)
row.names(dons_pourcentage_en_ligne)[nrow(dons_pourcentage_en_ligne)] <- "Total"
dons_pourcentage_en_ligne
```
...
```{r don_pourcentage_en_ligne}
don_pourcentage_en_ligne <- rowPercents(don)
kable(don_pourcentage_en_ligne)
```

```{r don_pourcentage_en_colonne}
don_pourcentage_en_colonne <- colPercents(don)
kable(don_pourcentage_en_colonne)
```




Comme nous le verrons plus loin, on passera en paramètre de la fonction R de calcul de l'AFC le tableau de contingence. Mais l'AFC travaille en réalité sur le tableau de probabilités que l'on peut calculer de cette manière sous R :

```{r tableau_de_probabilite}
# tableau_de_probabilite <- addmargins(don,FUN=list(Pourcentage=function(x) round(sum(x)/sum(don),2)))
# tableau_de_probabilite[length(tableau_de_probabilite)] <- 100
# tableau_de_probabilite
```
...
Pour clarifier cette notin d'indépendance, reprenons notre tableau de contingence avec ses marges.
...
```{r dons_avec_marges bis}
don_avec_marges
```


Affichons ce tableau de manière plus agréable dans ce document :
...
Le centre de l'axe correspond au profil moyen en ligne et au profil moyen en colonne. Pour rappel, le profil moyen correspond à la distribution marginale, c'est à dire à la marge (ligne ou colonne) divisée par l'effectif total (les 1724 femmes de l'enquête du CREDOC).
...
Rappel : L'AFC ne travaille pas sur le tableau de contingence mais transforme le tableau de contingence en tableau de probabilité. C'est à dire le tableau de contingence dont les valeurs sont divisées par la population. Deux points lignes sont donc proches l'un de l'autre si leur profil en pourcentage est similaire.       
